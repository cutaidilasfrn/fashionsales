        ...
        <div class="col-md-3 mb-3">
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    ...
                    <h3 class="fw-bold">{{ number_format($totalProduk) }}</h3>
                </div>
            </div>
        </div>
        <div class="col-md-3 mb-3">
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    ...
                    <h3 class="fw-bold">{{ number_format($totalPelanggan) }}</h3>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3">
                    <h5 class="card-title fw-bold mb-0 text-primary">
                        <i class="bi bi-graph-up-arrow me-2"></i>Tren Pendapatan Bulanan ({{ date('Y') }})
                    </h5>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-sm table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($produkTerlaris as $produk)
                                <tr>
                                    <td class="px-3">{{ $produk->nama_produk }}</td>
                                    <td class="text-end px-3">
                                        <span class="badge bg-success rounded-pill">{{ $produk->total }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        new Chart(platformCtx, {
            ...
            data: {
                labels: [
                    @foreach($platformTerlaris as $item)
                        '{{ $item->nama_platform }}',
                    @endforeach
                ],
                datasets: [{
